...
/*
 ...
 - 2019/XX/XX (1.XX) - when multi-viewports are enabled, all positions will be in your natural OS coordinates space. It means
 that:
                        - reference to hard-coded positions such as in SetNextWindowPos(ImVec2(0,0)) are probably not what you
 want anymore. you may use GetMainViewport()->Pos to offset hard-coded positions, e.g. SetNextWindowPos(GetMainViewport()->Pos)
                        - likewise io.MousePos and GetMousePos() will use OS coordinates.
                          If you query mouse positions to interact with non-imgui coordinates you will need to offset them, e.g.
 subtract GetWindowViewport()->Pos.
 ...
 - 2019/12/17 (1.75) - made Columns() limited to 64 columns by asserting above that limit. While the current code technically
 ...
 on this project.
    ...
    - Disclose your usage of Dear ImGui via a dev blog post, a tweet, a screenshot, a mention somewhere etc.
      You may post screenshot or links in the gallery threads (github.com/ocornut/imgui/issues/2847). Visuals are ideal as they
...
namespace ImGui {
    ...
    const ImGuiID IMGUI_VIEWPORT_DEFAULT_ID =
        0x11111111; // Using an arbitrary constant instead of e.g. ImHashStr("ViewportDefault", 0); so it's easier to spot in
                    // the debugger. The exact value doesn't matter.
    static ImGuiViewportP *AddUpdateViewport(ImGuiWindow *window, ImGuiID id, const ImVec2 &platform_pos, const ImVec2 &size,
...
void ImGuiIO::AddInputCharacterUTF16(ImWchar16 c) {
    ...
    {
        if (InputQueueSurrogate != 0) InputQueueCharacters.push_back(0xFFFD);
...
bool ImGui::IsItemHovered(ImGuiHoveredFlags flags) {
    ...
    if ((window->DC.LastItemId == window->ID || window->DC.LastItemId == window->MoveId) && window->WriteAccessed) return false;
    return true;
}
...
void ImGui::UpdateMouseMovingWindowEndFrame() {
    ...
    // (after we're done with all our widgets, so e.g. clicking on docking tab-bar which have set HoveredId already and not get
    // us here!)
...
static void NewFrameSanityChecks() {
    ...
    // Perform simple check: error if Docking or Viewport are enabled _exactly_ on frame 1 (instead of frame 0 or later), which
    // is a common error leading to loss of .ini data.
    ...
    if (g.IO.ConfigFlags & ImGuiConfigFlags_ViewportsEnable) {
        if ((g.IO.BackendFlags & ImGuiBackendFlags_PlatformHasViewports) &&
            ...
            IM_ASSERT((g.FrameCount == 0 || g.FrameCount == g.FrameCountPlatformEnded) &&
                      "Forgot to call UpdatePlatformWindows() in main loop after EndFrame()? Check examples/ applications for "
                      "reference.");
...
void ImGui::Initialize(ImGuiContext *context) {
    ...
    g.PlatformIO.MainViewport =
        g.Viewports[0]; // Make it accessible in public-facing GetPlatformIO() immediately (before the first call to EndFrame)
...
static void ImGui::EndFrameDrawDimmedBackgrounds() {
    ...
    if (dim_bg_for_window_list) {
        ...
        draw_list->AddRect(bb.Min, bb.Max, GetColorU32(ImGuiCol_NavWindowingHighlight, g.NavWindowingHighlightAlpha), rounding,
                           ~0, 3.0f);
...
void ImGui::Render() {
    ...
    for (int n = 0; n < g.Viewports.Size; n++) {
        ...
        // (note we scale cursor by current viewport/monitor, however Windows 10 for its own hardware cursor seems to be using a
...
static void FindHoveredWindow() {
    ...
    // Special handling for the window being moved: Ignore the mouse viewport check (because it may reset/lose its viewport
    // during the undocking frame)
    ...
    ImVec2 padding_for_resize_from_edges =
        g.IO.ConfigWindowsResizeFromEdges ? ImMax(g.Style.TouchExtraPadding, ImVec2(WINDOWS_RESIZE_FROM_EDGES_HALF_THICKNESS,
...
static bool ImGui::UpdateWindowManualResize(ImGuiWindow *window, const ImVec2 &size_auto_fit, int *border_held,
    ...
    // Clip mouse interaction rectangles within the viewport rectangle (in practice the narrowing is going to happen most of the
    // time).
    // - Not narrowing would mostly benefit the situation where OS windows _without_ decoration have a threshold for hovering
    // when outside their limits.
    //   This is however not the case with current back-ends under Win32, but a custom borderless window implementation would
    //   benefit from it.
    // - When decoration are enabled we typically benefit from that distance, but then our resize elements would be conflicting
    // with OS resize elements, so we also narrow.
    // - Note that we are unable to tell if the platform setup allows hovering with a distance threshold (on Win32, decorated
    // window have such threshold).
    ...
    for (int resize_grip_n = 0; resize_grip_n < resize_grip_count; resize_grip_n++) {
        ...
        if (hovered || held) g.MouseCursor = (resize_grip_n & 1) ? ImGuiMouseCursor_ResizeNESW : ImGuiMouseCursor_ResizeNWSE;
...
bool ImGui::Begin(const char *name, bool *p_open, ImGuiWindowFlags flags) {
    ...
    ImGuiWindow *parent_window_in_stack =
        window->DockIsActive ? window->DockNode->HostWindow : g.CurrentWindowStack.empty() ? NULL : g.CurrentWindowStack.back();
    ...
    if (window_just_appearing_after_hidden_for_resize && !(flags & ImGuiWindowFlags_ChildWindow)) window->NavLastIds[0] = 0;
    ...
    if (g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasWindowClass) window->WindowClass = g.NextWindowData.WindowClass;
    ...
    if (!(flags & ImGuiWindowFlags_DockNodeHost)) PushClipRect(window->InnerClipRect.Min, window->InnerClipRect.Max, true);
    ...
    // HiddenFramesCannotSkipItems. This will have the important effect of actually returning true in Begin() and not setting
    // SkipItems, allowing an earlier submission of the window contents. This is analogous to regular windows being hidden from
    // one frame. It is especially important as e.g. nested TabBars would otherwise generate flicker in the form of one empty
    // frame, or focus requests won't be processed.
    if (window->DockIsActive && !window->DockTabIsVisible) {
...
void ImGui::FocusWindow(ImGuiWindow *window) {
    ...
    if (focus_front_window->Flags &
        ...
        if (g.ActiveId != 0 && g.ActiveIdWindow && g.ActiveIdWindow->RootWindow != focus_front_window) ClearActiveID();
...
bool ImGui::IsWindowFocused(ImGuiFocusedFlags flags) {
    ...
    case ImGuiFocusedFlags_RootWindow: return g.NavWindow == g.CurrentWindow->RootWindowDockStop;
...
// surface declare their minimum size requirement to ItemSize() and then use a larger region for drawing/interaction, which is
// passed to ItemAdd().
...
static bool ImGui::NavScoreItem(ImGuiNavMoveResult *result, ImRect cand) {
    ...
    const ImRect &curr = g.NavScoringRectScreen; // Current modified source rect (NB: we've applied Max.x = Min.x in NavUpdate()
...
#undef NAV_MAP_KEY
    ...
    if (g.NavWindow && g.NavWindow->NavLastChildNavWindow != NULL && g.NavLayer == 0) g.NavWindow->NavLastChildNavWindow = NULL;
    ...
    g.NavScoringRectScreen =
        g.NavWindow ? ImRect(g.NavWindow->Pos + nav_rect_rel.Min, g.NavWindow->Pos + nav_rect_rel.Max) : ImRect(0, 0, 0, 0);
    g.NavScoringRectScreen.TranslateY(nav_scoring_rect_offset_y);
    g.NavScoringRectScreen.Min.x = ImMin(g.NavScoringRectScreen.Min.x + 1.0f, g.NavScoringRectScreen.Max.x);
    g.NavScoringRectScreen.Max.x = g.NavScoringRectScreen.Min.x;
    IM_ASSERT(!g.NavScoringRectScreen.IsInverted()); // Ensure if we have a finite, non-inverted bounding box here will allows
...
static float ImGui::NavUpdatePageUpPageDown() {
    ...
    if ((g.NavWindow->Flags & ImGuiWindowFlags_NoNavInputs) || g.NavWindowingTarget != NULL || g.NavLayer != 0) return 0.0f;
...
static void ImGui::NavUpdateWindowing() {
    ...
    if (apply_toggle_layer && g.NavWindow) {
        ...
        // When entering a regular menu bar with the Alt key, we always reinitialize the navigation ID. It however persist on
        // docking tab tabs.
        ...
        if (new_nav_layer == ImGuiNavLayer_Menu && !preserve_layer_1_nav_id) g.NavWindow->NavLastIds[ImGuiNavLayer_Menu] = 0;
...
static const char *GetFallbackWindowNameForWindowingList(ImGuiWindow *window) {
    ...
    if ((window->Flags & ImGuiWindowFlags_MenuBar) && strcmp(window->Name, "##MainMenuBar") == 0) return "(Main menu bar)";
...
static void WindowSettingsHandler_WriteAll(ImGuiContext *ctx, ImGuiSettingsHandler *handler, ImGuiTextBuffer *buf) {
    ...
    for (ImGuiWindowSettings *settings = g.SettingsWindows.begin(); settings != NULL;
        ...
        if (settings->Size.x != 0 || settings->Size.y != 0) buf->appendf("Size=%d,%d\n", settings->Size.x, settings->Size.y);
        buf->appendf("Collapsed=%d\n", settings->Collapsed);
        if (settings->DockId != 0) {
            // Write DockId as 4 digits if possible. Automatic DockId are small numbers, but full explicit DockSpace() are full
            // ImGuiID range.
...
// A) It won't take account of the possibility that non-imgui windows may be in-between our dragged window and our target
// window. B) It requires Platform_GetWindowFocus to be implemented by back-end.
...
// Note that this is running even if 'ImGuiConfigFlags_ViewportsEnable' is not set, in order to clear unused viewports (if any)
// and update monitor info.
static void ImGui::UpdateViewportsNewFrame() {
    ...
    if (viewports_enabled && (main_viewport->Flags & ImGuiViewportFlags_Minimized)) {
        main_viewport_pos = main_viewport->Pos; // Preserve last pos/size when minimized (FIXME: We don't do the same for Size
                                                // outside of the viewport path)
    ...
    // Mouse handling: decide on the actual mouse viewport for this frame between the active/focused viewport and the hovered
    // viewport. Note that 'viewport_hovered' should skip over any viewport that has the ImGuiViewportFlags_NoInputs flags set.
    ...
    if (g.IO.BackendFlags & ImGuiBackendFlags_HasMouseHoveredViewport) {
        ...
        // search: A) won't take account of the possibility that non-imgui windows may be in-between our dragged window and our
        // target window. B) uses LastFrameAsRefViewport as a flawed replacement for the last time a window was focused (we
        // could/should fix that by introducing Focus functions in PlatformIO)
        viewport_hovered = FindHoveredViewportFromPlatformWindowStack(g.IO.MousePos);
    }
    ...
    // - consider the case of holding on a menu item to browse child menus: even thou a mouse button is held, there's no active
    // id because menu items only react on mouse release.
...
static void ImGui::UpdateViewportsEndFrame() {
    ...
    g.Viewports[0]->ClearRequestFlags(); // Clear main viewport flags because UpdatePlatformWindows() won't do it and may not
                                         // even be called
}
...
static void ImGui::UpdateSelectWindowViewport(ImGuiWindow *window) {
    ...
    if ((g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasViewport) == 0) {
        ...
        // Attempt to restore saved viewport id (= window that hasn't been activated yet), try to restore the viewport based on
        // saved 'window->ViewportPos' restored from .ini file
...
void ImGui::UpdatePlatformWindows() {
    ...
    for (int i = 1; i < g.Viewports.Size; i++) {
        ...
        if (is_new_platform_window) {
            ...
            viewport->LastPlatformPos = viewport->LastPlatformSize =
                ImVec2(FLT_MAX, FLT_MAX); // By clearing those we'll enforce a call to Platform_SetWindowPos/Size below, before
    ...
    // io.MouseHoveredViewport. When setting Platform_GetWindowFocus, it is expected that the platform back-end can handle calls
    // without crashing if it doesn't have data stored.
...
// Custom renderers may prefer to not call this function at all, and instead iterate the publicly exposed platform data and
// handle rendering/sync themselves. The Render/Swap functions stored in ImGuiPlatformIO are merely here to allow for this
// helper to exist, but you can do it yourself:
//
...
static int ImGui::FindPlatformMonitorForRect(const ImRect &rect) {
    ...
    // Use a minimum threshold of 1.0f so a zero-sized rect won't false positive, and will still find the correct monitor given
    // its position. This is necessary for tooltips which always resize down to zero at first.
...
void ImGui::DestroyPlatformWindows() {
    ...
    // It is convenient for the platform back-end code to store something in the main viewport, in order for e.g. the mouse
    // handling code to operator a consistent manner. It is expected that the back-end can handle calls to
...
struct ImGuiDockRequest {
    ...
    ImGuiWindow *DockTargetWindow; // Destination/Target Window to dock into (may be a loose window or a DockNode, might be NULL
                                   // in which case DockTargetNode cannot be NULL)
...
namespace ImGui {
    ...
    static bool DockNodeCalcDropRectsAndTestMousePos(const ImRect &parent, ImGuiDir dir, ImRect &out_draw, bool outer_docking,
    ...
    static void DockNodeTreeSplit(ImGuiContext *ctx, ImGuiDockNode *parent_node, ImGuiAxis split_axis, int split_first_child,
...
// ImGuiDockNodeSettings, or we always hold the entire docking node tree. Nodes are frequently hidden, e.g. if the window(s) or
// child nodes they host are not active. At boot time only, we run a simple GC to remove nodes that have no references. Because
// dock node settings (which are small, contiguous structures) are always mirrored by their corresponding dock nodes (more
// complete structures), we can also very easily recreate the nodes from scratch given the settings data (this is what
// DockContextRebuild() does). This is convenient as docking reconfiguration can be implemented by mostly poking at the simpler
// settings data.
//-----------------------------------------------------------------------------
...
void ImGui::DockContextUpdateDocking(ImGuiContext *ctx) {
    ...
    // We can have NULL pointers when we delete nodes, but because ID are recycled this should amortize nicely (and our node
    // count will never be very high)
    for (int n = 0; n < dc->Nodes.Data.Size; n++)
        if (ImGuiDockNode *node = (ImGuiDockNode *)dc->Nodes.Data[n].val_p)
...
ImGuiID ImGui::DockContextGenNodeID(ImGuiContext *ctx) {
    ...
    // FIXME-OPT FIXME-DOCK: This is suboptimal, even if the node count is small enough not to be a worry. We should poke in
    // ctx->Nodes to find a suitable ID faster.
    ImGuiID id = 0x0001;
...
static ImGuiDockNode *ImGui::DockContextAddNode(ImGuiContext *ctx, ImGuiID id) {
    // Generate an ID for the new node (the exact ID value doesn't matter as long as it is not already used) and add the first
    // window.
    if (id == 0)
...
static void ImGui::DockContextPruneUnusedSettingsNodes(ImGuiContext *ctx) {
    ...
    // We track the 'auto-DockNode <- manual-Window <- manual-DockSpace' in order to avoid 'auto-DockNode' being ditched by
    // DockContextPruneUnusedSettingsNodes()
    for (int settings_n = 0; settings_n < dc->SettingsNodes.Size; settings_n++) {
        ImGuiDockNodeSettings *settings = &dc->SettingsNodes[settings_n];
    ...
    for (int settings_n = 0; settings_n < dc->SettingsNodes.Size; settings_n++) {
        ...
        remove |=
            (data->CountWindows == 0 && settings->ParentNodeId == 0 && data->CountChildNodes == 0); // Leaf nodes with 0 window
...
static void ImGui::DockContextBuildNodesFromSettings(ImGuiContext *ctx, ImGuiDockNodeSettings *node_settings_array,
    ...
    for (int node_n = 0; node_n < node_settings_count; node_n++) {
        ...
        // This is useful as the RootWindowForTitleBarHighlight links necessary to highlight the currently focused node requires
        // node->HostWindow to be set.
...
void ImGui::DockContextQueueDock(ImGuiContext *ctx, ImGuiWindow *target, ImGuiDockNode *target_node, ImGuiWindow *payload,
...
void ImGui::DockContextProcessDock(ImGuiContext *ctx, ImGuiDockRequest *req) {
    ...
    // When processing an interactive split, usually LastFrameAlive will be < g.FrameCount. But DockBuilder operations can make
    // it ==.
    ...
    if (node != payload_node) {
        // Create tab bar before we call DockNodeMoveWindows (which would attempt to move the old tab-bar, which would lead us
        // to payload tabs wrongly appearing before target tabs!)
        ...
        if (payload_node != NULL) {
            ...
            if (payload_node->IsSplitNode()) {
                if (node->Windows.Size > 0) {
                    // We can dock a split payload into a node that already has windows _only_ if our payload is a node tree
                    // with a single visible node. In this situation, we move the windows of the target node into the currently
                    // visible node of the payload. This allows us to preserve some of the underlying dock tree settings nicely.
...
void ImGui::DockContextProcessUndockNode(ImGuiContext *ctx, ImGuiDockNode *node) {
    ...
    if (node->IsRootNode() || node->IsCentralNode()) {
        ...
        node->ParentNode->AuthorityForViewport =
            ImGuiDataAuthority_Window; // The node that stays in place keeps the viewport, so our newly dragged out node will
...
static void ImGui::DockNodeAddWindow(ImGuiDockNode *node, ImGuiWindow *window, bool add_to_tab_bar) {
    ...
    // storage. In particular it is important we init the viewport from the first window so we don't create two viewports and
    // drop one.
    ...
    // Update this without waiting for the next time we Begin() in the window, so our host window will have the proper title bar
    // color on its first frame.
    if (node->HostWindow)
...
static void ImGui::DockNodeRemoveWindow(ImGuiDockNode *node, ImGuiWindow *window, ImGuiID save_dock_id) {
    ...
    // Update visibility immediately is required so the DockNodeUpdateRemoveInactiveChilds() processing can reflect changes up
    // the tree
    DockNodeUpdateVisibleFlag(node);
}
...
static void ImGui::DockNodeApplyPosSizeToWindows(ImGuiDockNode *node) {
    for (int n = 0; n < node->Windows.Size; n++) {
        SetWindowPos(node->Windows[n], node->Pos, ImGuiCond_Always); // We don't assign directly to Pos because it can break the
...
static void ImGui::DockNodeUpdateVisibleFlagAndInactiveChilds(ImGuiDockNode *node) {
    ...
    // There is the possibility that one of our child becoming empty will delete itself and moving its sibling contents into
    // 'node'. If 'node->ChildNode[0]' delete itself, then 'node->ChildNode[1]->Windows' will be moved into 'node' If
    // 'node->ChildNode[1]' delete itself, then 'node->ChildNode[0]->Windows' will be moved into 'node' and the "remove inactive
...
static void ImGui::DockNodeUpdateForRootNode(ImGuiDockNode *node) {
    ...
    // - Find if there's only a single visible window in the hierarchy (in which case we need to display a regular title bar ->
    // FIXME-DOCK: that last part is not done yet!)
...
static void ImGui::DockNodeUpdate(ImGuiDockNode *node) {
    ...
    // Early out for hidden root dock nodes (when all DockId references are in inactive windows, or there is only 1 floating
    // window holding on the DockId)
    ...
    // We could remove this frame if we could reliably calculate the expected window size during node update, before the Begin()
    // code. It would require a generalization of CalcWindowExpectedSize(), probably extracting code away from Begin(). In
    // reality it isn't very important as user quickly ends up with size data in .ini file.
    ...
    if (node->IsDockSpace()) {
        ...
        if (node->IsRootNode() && node->IsVisible) {
            ...
            // display pile, but they are not linked to the dock host window during the frame they appear. The dock host window
            // would keep its old display order, and the sorting in EndFrame would move B/C/D back after the dock host window,
            // losing their top-most status.
    ...
    // We need to draw a background at the root level if requested by ImGuiDockNodeFlags_PassthruCentralNode, but we will only
    // know the correct pos/size after processing the resizing splitters. So we are using the DrawList channel splitting
    // facility to submit drawing primitives out of order!
    ...
    if (central_node_hole_register_hit_test_hole) {
        ...
        IM_ASSERT(node->IsDockSpace()); // We cannot pass this flag without the DockSpace() api. Testing this because we also
    ...
    if (render_dockspace_bg && node->IsVisible) {
        ...
        if (central_node_hole)
            RenderRectFilledWithHole(host_window->DrawList, node->Rect(), central_node->Rect(), GetColorU32(ImGuiCol_WindowBg),
                                     0.0f);
        else
...
// Compare TabItem nodes given the last known DockOrder (will persist in .ini file as hint), used to sort tabs when multiple
// tabs are added on the same frame.
static int IMGUI_CDECL TabItemComparerByDockOrder(const void *lhs, const void *rhs) {
    ImGuiWindow *a = ((const ImGuiTabItem *)lhs)->Window;
    ImGuiWindow *b = ((const ImGuiTabItem *)rhs)->Window;
    if (int d = ((a->DockOrder == -1) ? INT_MAX : a->DockOrder) - ((b->DockOrder == -1) ? INT_MAX : b->DockOrder)) return d;
...
static void ImGui::DockNodeUpdateTabBar(ImGuiDockNode *node, ImGuiWindow *host_window) {
    ...
    // Move ourselves to the Menu layer (so we can be accessed by tapping Alt) + undo SkipItems flag in order to draw over the
    // title bar even if the window is collapsed
    ...
    host_window->DrawList->AddRectFilled(title_bar_rect.Min, title_bar_rect.Max, title_bar_col, host_window->WindowRounding,
                                         ImDrawCornerFlags_Top);
    ...
    // Submit new tabs and apply NavWindow focus back to the tab bar. They will be added as Unsorted and sorted below based on
    ...
    for (int tab_n = tab_bar->Tabs.Size - 1; tab_n >= 0 && (tab_bar->Tabs[tab_n].Flags & ImGuiTabItemFlags_Unsorted); tab_n--) {
        // FIXME-DOCK: Consider only clearing the flag after the tab has been alive for a few consecutive frames, allowing late
        // comers to not break sorting?
    ...
    ImGuiTabBarFlags tab_bar_flags =
        ImGuiTabBarFlags_Reorderable | ImGuiTabBarFlags_AutoSelectNewTabs; // | ImGuiTabBarFlags_NoTabListScrollingButtons);
...
static void ImGui::DockNodeCalcTabBarLayout(const ImGuiDockNode *node, ImRect *out_title_rect, ImRect *out_tab_bar_rect,
    ...
    ImGuiContext &g = *GImGui;
    ImRect r =
        ImRect(node->Pos.x, node->Pos.y, node->Pos.x + node->Size.x, node->Pos.y + g.FontSize + g.Style.FramePadding.y * 2.0f);
    ...
    if (node->HasCloseButton) {
        r.Max.x -= g.FontSize; // +1.0f; // In DockNodeUpdateTabBar() we currently display a disabled close button even if there
                               // is none.
    }
...
static void ImGui::DockNodePreviewDockSetup(ImGuiWindow *host_window, ImGuiDockNode *host_node, ImGuiWindow *root_payload,
    ...
    // Build a tentative future node (reuse same structure because it is practical. Shape will be readjusted when previewing a
    // split)
    ...
    // When docking without holding Shift, we only allow and preview docking when hovering over a drop rect or over the title
    // bar
...
static void ImGui::DockNodePreviewDockRender(ImGuiWindow *host_window, ImGuiDockNode *host_node, ImGuiWindow *root_payload,
...
void ImGui::DockNodeTreeUpdatePosSize(ImGuiDockNode *node, ImVec2 pos, ImVec2 size, bool only_write_to_marked_nodes) {
    ...
    if (child_0->IsVisible && child_1->IsVisible) {
        ...
        // 2) Process locked absolute size (during a splitter resize we preserve the child of nodes not touching the splitter
        // edge)
        ...
        // 3) If one window is the central node (~ use remaining space, should be made explicit!), use explicit size from the
        // other, and remainder for the central node
                        ...
                        for (int touching_node_n = 0; touching_node_n < touching_nodes[side_n].Size; touching_node_n++) {
                            ...
                            while (touching_node->ParentNode != node) {
                                if (touching_node->ParentNode->SplitAxis == axis) {
                                    ...
                                    // draw_list->AddRect(touching_node->Pos, touching_node->Rect().Max, IM_COL32(255, 0, 0,
                                    // 255)); draw_list->AddRectFilled(node_to_preserve->Pos, node_to_preserve->Rect().Max,
...
ImGuiDockNode *ImGui::DockNodeTreeFindNodeByPos(ImGuiDockNode *node, ImVec2 pos) {
    ...
    if (node->IsDockSpace() && node->IsRootNode()) {
        if (node->CentralNode && node->IsLeafNode()) // FIXME-20181220: We should not have to test for IsLeafNode() here but we
...
// default. The Central Node is always displayed even when empty and shrink/extend according to the requested size of its
// neighbors. DockSpace() needs to be submitted _before_ any window they can host. If you use a dockspace, submit it early in
// your app.
void ImGui::DockSpace(ImGuiID id, const ImVec2 &size_arg, ImGuiDockNodeFlags flags, const ImGuiWindowClass *window_class) {
    ...
    // This is faster but also DockNodeUpdateTabBar() relies on TabBarLayout() running (which won't if SkipItems=true) to set
    // NextSelectedTabId = 0). See #2960. If for whichever reason this is causing problem we would need to ensure that
    ...
    // It is possible that the node has already been claimed by a docked window which appeared before the DockSpace() node, so
    // we overwrite IsDockSpace again.
...
// Even though we could pass window flags, it would also require the user to be able to call BeginMenuBar() somehow meaning we
// can't Begin/End in a single function. But you can also use BeginMainMenuBar(). If you really want a menu bar inside the same
...
// Make sure to use the ImGuiDockNodeFlags_DockSpace flag to create a dockspace node! Otherwise this will create a floating
// node!
...
//   For various reason, the splitting code currently needs a base size otherwise space may not be allocated as precisely as you
//   would expect.
...
void ImGui::DockBuilderRemoveNodeChildNodes(ImGuiID root_id) {
    ...
    // DockNodeMoveWindows->DockNodeAddWindow will normally set those when reaching two windows (which is only adequate during
    // interactive merge) Make sure we don't lose our current pos/size. (FIXME-DOCK: Consider tidying up that code in
    // DockNodeAddWindow instead)
    if (root_node) {
    ...
    // Not really efficient, but easier to destroy a whole hierarchy considering DockContextRemoveNode is attempting to merge
    // nodes
...
void ImGui::DockBuilderCopyDockSpace(ImGuiID src_dockspace_id, ImGuiID dst_dockspace_id,
    ...
    // FIXME: When overwriting dst_dockspace_id, windows that aren't part of our dockspace window class but that are docked in a
    // same node will be split apart, whereas we could attempt to at least keep them together in a new, same floating node.
    ImVector<ImGuiID> node_remap_pairs;
    DockBuilderCopyNode(src_dockspace_id, dst_dockspace_id, &node_remap_pairs);

    // Attempt to transition all the upcoming windows associated to dst_dockspace_id into the newly created hierarchy of dock
    // nodes (The windows associated to src_dockspace_id are staying in place)
    ...
    // Anything else in the source nodes of 'node_remap_pairs' are windows that were docked in src_dockspace_id but are not
    // owned by it (unaffiliated windows, e.g. "ImGui Demo") Find those windows and move to them to the cloned dock node. This
    // may be optional?
    for (int dock_remap_n = 0; dock_remap_n < node_remap_pairs.Size; dock_remap_n += 2)
...
static ImGuiDockNode *ImGui::DockContextBindNodeToWindow(ImGuiContext *ctx, ImGuiWindow *window) {
    ...
    // (often it will be the root node). If we don't do this, the window will be assigned a zero-size on its first frame, which
    // won't ideally warm up the layout. This is a little wonky because we don't normally update the Pos/Size of visible node
    // mid-frame.
    if (!node->IsVisible) {
...
void ImGui::BeginDocked(ImGuiWindow *window, bool *p_open) {
    ...
    // Note how we are testing for LastFrameAlive and NOT LastFrameActive. A DockSpace node can be maintained alive while being
    // inactive with ImGuiDockNodeFlags_KeepAliveOnly.
    ...
    // FIXME-DOCK: replace ->HostWindow NULL compare with something more explicit (~was initially intended as a first frame
    // test)
    ...
    if (node->IsHiddenTabBar() || node->IsNoTabBar())
        ...
        window->Flags &=
            ~ImGuiWindowFlags_NoTitleBar; // Clear the NoTitleBar flag in case the user set it: confusingly enough we need a
...
void ImGui::BeginDockableDragDropSource(ImGuiWindow *window) {
    ...
    bool is_drag_docking =
        (g.IO.ConfigDockingWithShift) || ImRect(0, 0, window->SizeFull.x, GetFrameHeight()).Contains(g.ActiveIdClickOffset);
    if (is_drag_docking &&
...
void ImGui::BeginDockableDragDropTarget(ImGuiWindow *window) {
    ...
    if (AcceptDragDropPayload(IMGUI_PAYLOAD_TYPE_WINDOW,
        ...
        // const bool do_preview = true;     // Ignore testing for payload->IsPreview() which removes one frame of delay, but
        // breaks overlapping drop targets within the same window.
...
static void DockSettingsHandler_DockNodeToSettings(ImGuiDockContext *dc, ImGuiDockNode *node, int depth) {
    ...
    node_settings.ParentWindowId =
        (node->IsDockSpace() && node->HostWindow && node->HostWindow->ParentWindow) ? node->HostWindow->ParentWindow->ID : 0;
...
void ImGui::ShowViewportThumbnails() {
    ...
    // We don't display full monitor bounds (we could, but it often looks awkward), instead we display just enough to cover all
    // of our viewports.
    ...
    // for (int n = 0; n < g.PlatformIO.Monitors.Size; n++)
    //    window->DrawList->AddRect(off + g.PlatformIO.Monitors[n].MainPos * SCALE, off + (g.PlatformIO.Monitors[n].MainPos +
    //    g.PlatformIO.Monitors[n].MainSize) * SCALE, ImGui::GetColorU32(ImGuiCol_Border));
    for (int n = 0; n < g.Viewports.Size; n++) {
        ImGuiViewportP *viewport = g.Viewports[n];
    ...
    struct Funcs {
        ...
        static void NodeViewport(ImGuiViewportP *viewport) {
            ...
            if (ImGui::TreeNode((void *)(intptr_t)viewport->ID, "Viewport #%d, ID: 0x%08X, Parent: 0x%08X, Window: \"%s\"",
                ...
                ImGui::BulletText("Main Pos: (%.0f,%.0f), Size: (%.0f,%.0f)\nWorkArea Offset Left: %.0f Top: %.0f, Right: "
                                  ...
                                  viewport->WorkOffsetMax.y, viewport->PlatformMonitor, viewport->DpiScale * 100.0f);
                if (viewport->Idx > 0) {
                    ImGui::SameLine();
        ...
        static void NodeDockNode(ImGuiDockNode *node, const char *label) {
            ...
            if (node->Windows.Size > 0)
                ...
                open = ImGui::TreeNode((void *)(intptr_t)node->ID, "%s 0x%04X%s: %s split (vis: '%s')", label, node->ID,
        ...
        if (open) {
            for (int i = 0; i < g.PlatformIO.Monitors.Size; i++) {
                ...
                ImGui::BulletText("Monitor #%d: DPI %.0f%%\n MainMin (%.0f,%.0f), MainMax (%.0f,%.0f), MainSize (%.0f,%.0f)\n "
                                  ...
                                  mon.WorkPos.x + mon.WorkSize.x, mon.WorkPos.y + mon.WorkSize.y, mon.WorkSize.x,
                                  mon.WorkSize.y);
            }
            ImGui::TreePop();
        }
